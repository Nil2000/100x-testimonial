  ...
  return (
    ...
    >
      ...
      <div className="lg:w-64 flex-shrink-0">
        <TabsList className="flex lg:flex-col h-auto w-full bg-muted/50 p-1.5 rounded-lg gap-1">
          {tabs.map((tab, index) => {
      ...
      <div className="flex-1 min-w-0">
        {tabs.map((tab, index) => (
          <TabsContent
            value={`tab-${index}`}
            key={`tab-${index}`}
            className="mt-0 min-h-[calc(100vh-16rem)] animate-in fade-in-50 slide-in-from-right-2 duration-300"
          >
